...
class BarPlacementTests(unittest.TestCase):
    ...
    def test_placement_only_island_appears_on_its_selected_edge(self):
        # Shelf spawning writes a placement before the island has ever been
        # part of the base layout.
        raw = json.dumps({"DP-1": {"tray": {"edge": "top", "zone": "start", "order": 1}}})
        top = self.evaluate(f'layoutFor(base, "top", "top", {json.dumps(raw)}, "DP-1")')
        left = self.evaluate(f'layoutFor(base, "top", "left", {json.dumps(raw)}, "DP-1")')
        self.assertEqual(top["start"], ["launcher", "workspaces", "tray"])
        self.assertNotIn("tray", left["start"])
