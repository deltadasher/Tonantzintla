...
const label=el(),status=el(),copy=el(),watch=el(),close=el();
...
const document={body,querySelectorAll:s=>s==='[data-instrument]'?tabs:s==='.swatches button[data-tone]'?tones:[],getElementById:id=>id==='tone-label'?label:panels[Number(id.split('-')[1])],querySelector:s=>({'#demo-dialog':dialog,'[data-watch]':watch,'#copy-install':copy,'#install-commands':code,'#copy-status':status}[s]),createRange:()=>({selectNodeContents(){}})};
...
tones[1].events.click();assert.equal(body.dataset.tone,'lilac');assert.equal(tones[1].attrs['aria-pressed'],'true');assert(label.textContent.includes('LUNAR'));
