...
function layoutFor(baseLayout, primaryEdge, targetEdge, raw, outputName) {
    ...
    // The shelf can introduce an island which has never appeared in a base
    // layout. Its placement is its only source of membership, so merge it
    // here rather than silently dropping it from the live bar model.
    Object.keys(placements).forEach(function(id) {
        if (baseIds[id]) return;
        var entry = placements[id];
        if (!validPlacement(entry) || entry.edge !== targetEdge
                || zones.indexOf(entry.zone) < 0) return;
        var order = Number(entry.order);
        buckets[entry.zone].push({
            id: id,
            order: Number.isFinite(order) ? order : sequence,
            sequence: sequence++
        });
    });
